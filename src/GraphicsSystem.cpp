...
void GraphicsSystem::init(int window_width, int window_height, std::string assets_folder) {
	...
	light_shader_ = new Shader("data/shaders/light.vert", "data/shaders/light.frag");
	...
	deferred_shader_ = new Shader("data/shaders/new-phong.vert", "data/shaders/new-phong.frag");

	mix_shader_ = new Shader("data/shaders/mix.vert", "data/shaders/mix.frag");

	cone_geom_ = createGeometryFromFile("data/assets/cone.obj");

	gbuffer_.initGbuffer(viewport_width_, viewport_height_);
	utility_buffer_.initUtilityBuffer(viewport_width_, viewport_height_);
}
...
void GraphicsSystem::lateInit() {
	...
	for (size_t i = 0; i < ECS.getAllComponents<Light>().size(); i++) {
		...
		volumetric_frame_[i].initLight(2048, 2048);
	}
...
void GraphicsSystem::update(float dt) {
	...
	lm::mat4 vp = cam.view_projection;
	useShader(light_shader_);
	for (size_t i = 0; i < lights.size(); i++) {
		if (lights[i].type == 2) {

			volumetric_frame_[i].bindAndClear();
			
			Transform &trans = ECS.getComponentFromEntity<Transform>(lights[i].owner);
			lm::vec3 position = trans.position();
			lm::mat4 model;
			model.scale(lm::vec3(0.25 * lights[i].spot_inner, -20, -0.25 * lights[i].spot_inner));
			model.translate(position);
			lm::mat4 mvp = vp * model;
			lm::mat4 normal_matrix = model;
			normal_matrix.inverse();
			normal_matrix.transpose();


			shader_->setUniform(U_NUM_LIGHTS, (int)ECS.getAllComponents<Light>().size());
			shader_->setUniform(U_MVP, mvp);
			shader_->setUniform(U_VP, vp);
			...
			shader_->setUniform(L_DIRECTION, lights[i].direction);
			shader_->setUniform(L_TYPE, lights[i].type);
			shader_->setUniform(L_POSITION, lights[i].position);
			shader_->setUniform(L_COLOR, lights[i].color);
			shader_->setUniform(L_LINEAR_ATT, lights[i].linear_att);
			shader_->setUniform(L_QUADRATIC_ATT, lights[i].quadratic_att);
			shader_->setUniform(L_VIEW_PROJECTION, lights[i].view_projection);
			shader_->setTexture(L_TEX_DEPTH, shadow_frame_[i].color_textures[0], 0);
			shader_->setTexture(U_TEX_DEPTH, gbuffer_.color_textures[4], 0);
			geometries_[cone_geom_].render();
			glBindFramebuffer(GL_READ_FRAMEBUFFER, gbuffer_.framebuffer);
			glBindFramebuffer(GL_DRAW_FRAMEBUFFER, volumetric_frame_[i].framebuffer);
			glBlitFramebuffer(
				0, 0, viewport_width_, viewport_height_, 0, 0, viewport_width_, viewport_height_,
	...
	/* SCREEN PASS */
	utility_buffer_.bindAndClear();
	renderGbuffer(utility_buffer_.framebuffer);
	addUnifromShaderMaps(5);
	renderEnvironment_();
 ...
 //   }
	...
	bindAndClearScreen_();
	useShader(mix_shader_);
	mix_shader_->setUniform(U_NUM_LIGHTS, (int)lights.size());
	mix_shader_->setTexture(U_TEX_PHONG, utility_buffer_.color_textures[0], 0);
	mix_shader_->setTexture(U_TEX_BLOOM, utility_buffer_.color_textures[1], 1);
	mix_shader_->setTexture(U_TEX_BACKGROUND, utility_buffer_.color_textures[2], 2);

	for (size_t i = 0; i < lights.size(); i++) {

		glActiveTexture(GL_TEXTURE0 + (GLenum)i + 3);
		glBindTexture(GL_TEXTURE_2D, volumetric_frame_[i].color_textures[0]);

		std::string volumetric_map_name = "u_volumetric_map[" + std::to_string(i) + "]";
		GLint u_volumetric_map_pos = glGetUniformLocation(shader_->program, volumetric_map_name.c_str());
		if (u_volumetric_map_pos != -1)
			glUniform1i(u_volumetric_map_pos, i + 3);
	}

	geometries_[screen_space_geom_].render();
	glBindFramebuffer(GL_READ_FRAMEBUFFER, utility_buffer_.framebuffer);
...
void GraphicsSystem::renderGbuffer(GLuint depthToBuffer) {
	...
	deferred_shader_->setTexture(U_TEX_BLOOM, gbuffer_.color_textures[3], 3);
	deferred_shader_->setTexture(U_TEX_AMBIENT, gbuffer_.color_textures[5], 4);
	...
	glBindFramebuffer(GL_READ_FRAMEBUFFER, gbuffer_.framebuffer);
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, depthToBuffer);
	glBlitFramebuffer(
		0, 0, viewport_width_, viewport_height_, 0, 0, viewport_width_, viewport_height_,
		GL_DEPTH_BUFFER_BIT, GL_NEAREST
	);
...
//renders a mesh from a Light/Camera, only setting its MVP
//i.e. only usable with a depth shader
//void GraphicsSystem::renderVolumetric_(Mesh& comp, const Light& light) {
//	//get transform and matrices
//	Transform& transform = ECS.getComponentFromEntity<Transform>(comp.owner);
//	lm::mat4 model_matrix = transform.getGlobalMatrix(ECS.getAllComponents<Transform>());
//	lm::mat4 mvp_matrix = light.view_projection * model_matrix;
...
void GraphicsSystem::setMaterialUniforms() {
	...
	shader_->setUniform(U_BLOOM, mat.bloom ? 1 : 0);
    //texture uniforms
    ...
    if (mat.cube_map) {
        ...
        shader_->setTextureCube(U_SKYBOX, mat.cube_map, 9);
...
void GraphicsSystem::addUnifromShaderMaps(int textureStratingPosition) {
	auto lights = ECS.getAllComponents<Light>();
	for (size_t i = 0; i < lights.size(); i++) {

		glActiveTexture(GL_TEXTURE0 + (GLenum)i + textureStratingPosition);
		glBindTexture(GL_TEXTURE_2D, shadow_frame_[i].color_textures[0]);

		std::string shadow_map_name = "u_shadow_map[" + std::to_string(i) + "]";
		GLint u_shadow_map_pos = glGetUniformLocation(shader_->program, shadow_map_name.c_str());
		if (u_shadow_map_pos != -1)
			glUniform1i(u_shadow_map_pos, i + textureStratingPosition);
	}
