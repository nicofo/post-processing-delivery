...
void Framebuffer::getDephtFrom(Framebuffer buffer) {
...
void Framebuffer::initLight(GLsizei w, GLsizei h) {
	width = w; height = h;
	//create and bind
	glGenFramebuffers(1, &(framebuffer));
	glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
...
void Framebuffer::initGbuffer(GLsizei w, GLsizei h) {
	...
	float borderColor[] = { 1.0f, 1.0f, 1.0f, 1.0f };
	glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, borderColor);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT4, GL_TEXTURE_2D, color_textures[4], 0);
...
void Framebuffer::initUtilityBuffer(GLsizei w, GLsizei h) {
	width = w; height = h;
	//create and bind
	glGenFramebuffers(1, &(framebuffer));
	glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
